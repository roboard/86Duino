...
public class PdePreprocessor {
  ...
  private void chngeIncludeData(String Path){
    try{
  ...
  public int writePrefix(String program, String buildPath,
                         String sketchName, String codeFolderPackages[],String sketchNamePath) throws FileNotFoundException {
    this.buildPath = buildPath;
    this.name = sketchName;
        
    try{
    Compiler compiler = new Compiler();
    prefs = compiler.createBuildPreferences(buildPath, sketchName);
    String packageName = prefs.get("target_package");
    sketchNamePath = prefs.get("runtime.ide.path");

    if(packageName.compareTo("86duino") == 0){
      chngeIncludeData(sketchNamePath+File.separator+"DJGPP");
      for(int i = 0; i < IncludeName.size() ; i+=2){
        program=program.replaceAll(IncludeName.get(i),"\n#include <"+IncludeName.get(i+1)+">\n"); 
      }
    }
    }catch(Exception e){}  
