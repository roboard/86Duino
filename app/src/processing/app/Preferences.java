...
public class Preferences {
  ...
  static protected void init(File file) {
    ...
    try {
      ...
      Base.showError(null, _("Could not read default settings.\n" +
                             "You'll need to reinstall 86Duino."), e);
    }
  ...
  public Preferences() {
    ...
    // Preferred language: [        ] (requires restart of 86Duino)
    ...
    label = new JLabel(_("  (requires restart of 86Duino)"));
    box.add(label);
    pain.add(box);
    ...
    if (Base.isWindows()) {
      autoAssociateBox =
        new JCheckBox(_("Automatically associate .ino files with 86Duino"));
    ...
    label = new JLabel(_("(edit only when 86Duino is not running)"));
    pain.add(label);
    d = label.getPreferredSize();
    label.setForeground(Color.gray);
    label.setBounds(left, top, d.width, d.height);
