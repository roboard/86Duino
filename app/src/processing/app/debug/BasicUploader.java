  ...
  public boolean uploadUsingPreferences(String buildPath, String className,
    ...
    if (usingProgrammer || prefs.get("upload.protocol") == null) {
      ...
      if(packageName.compareTo("86duino") == 0){
        RunnerException re = new RunnerException(_("This function just for Arduino!!"));
        re.hideStackTrace();
        throw re;
        //return false;
      }  
...
//      }
  ...
  private String waitForUploadPort(String uploadPort, List<String> before)
    ...
    while (elapsed < 20000) {
      ...
      if (((!Base.isWindows() && elapsed >= 500) || elapsed >= 10000) &&
  ...
  public boolean burn86Bootloader() throws RunnerException,SerialException {
    TargetPlatform targetPlatform = Base.getTargetPlatform();
    PreferencesMap prefs = Preferences.getMap();
    prefs.putAll(Base.getBoardPreferences());
    prefs.putAll(targetPlatform.getTool(prefs.get("upload.tool")));
