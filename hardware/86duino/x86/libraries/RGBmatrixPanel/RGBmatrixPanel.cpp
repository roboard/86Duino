 ...
 **********************************************************************/
 #define GPIOP4DATA_ADDR    0xF210
 #define GPIOP3DATA_ADDR    0xF20C
 #define GPIOP2DATA_ADDR    0xF208
 
 void myisr (void);
 
 void setPort2DIR (uint8_t byte) {
	 // set GPIO DIR register
	 outportb(GPIODIRBASE + 8, (inportb(GPIODIRBASE + 8) & 0xC0) | (byte & 0x3F)); // 8 = 4*2
 }
 
 DMP_INLINE(void) setPort2DATA (uint8_t byte) {
	 static bool inited = false;
	 // set GPIO DIR register
	 outportb(GPIOP2DATA_ADDR, (inportb(GPIOP2DATA_ADDR) & 0xC0) | (byte & 0x3F)); // 8 = 4*2
	 // set crossbar register to GPIO function (for 8 pins)
	 if (inited == false)
	 {
		 for (int i=0; i<8; i++) outportb(CROSSBARBASE + 0xA0, 0x01);
		 inited = true;
	 }
 }
...
void RGBmatrixPanel::begin(void) {
  ...
  #ifndef _86DUINO
  DATADIR  = B11111100; // Bit 2 ~ 7, 6-bits
  DATAPORT = 0;
  #else
  setPort2DIR(0x3F); // Bit 0 ~ 5, 6-bits
  setPort2DATA(0x00);
  #endif
...
uint16_t RGBmatrixPanel::Color333(uint8_t r, uint8_t g, uint8_t b) {
  // RRRrrGGGgggBBBbb
  return (((uint16_t)r & 0x7) << 13) | (((uint16_t)r & 0x6) << 10) |
         (((uint16_t)g & 0x7) <<  8) | (((uint16_t)g & 0x7) <<  5) |
         (((uint16_t)b & 0x7) <<  2) | (((uint16_t)b & 0x6) >>  1);
}

// Promote 4/4/4 RGB to Adafruit_GFX 5/6/5
uint16_t RGBmatrixPanel::Color444(uint8_t r, uint8_t g, uint8_t b) {
  // RRRRrGGGGggBBBBb
  return (((uint16_t)r & 0xF) << 12) | (((uint16_t)r & 0x8) << 8) |
         (((uint16_t)g & 0xF) <<  7) | (((uint16_t)g & 0xC) << 3) |
         (((uint16_t)b & 0xF) <<  1) | ((b & 0x8) >> 3);
}
...
uint16_t RGBmatrixPanel::Color888(
  ...
  if(gflag) { // Gamma-corrected color?
    r = pgm_read_byte(&_gamma[r]); // Gamma correction table maps
    g = pgm_read_byte(&_gamma[g]); // 8-bit input to 4-bit output
    b = pgm_read_byte(&_gamma[b]);
...
uint16_t RGBmatrixPanel::ColorHSV(
  ...
  v1 = val + 1;
  if(gflag) { // Gamma-corrected color?
    r = pgm_read_byte(&_gamma[(r * v1) >> 8]); // Gamma correction table maps
    g = pgm_read_byte(&_gamma[(g * v1) >> 8]); // 8-bit input to 4-bit output
    b = pgm_read_byte(&_gamma[(b * v1) >> 8]);
  } else { // linear (uncorrected) color
...
void RGBmatrixPanel::swapBuffers(boolean copy) {
  if(matrixbuff[0] != matrixbuff[1]) {
	...
	while(swapflag == true) delay(50); // wait for interrupt to clear it **** value 50 just for 86Duino !! ****
    if(copy == true)
...
void RGBmatrixPanel::updateDisplay(void) {
  ...
  outportb(GPIOP4DATA_ADDR, inportb(GPIOP4DATA_ADDR)|0x08); // digitalWrite(_oe, HIGH); Disable LED output during row/plane switchover
  outportb(GPIOP3DATA_ADDR, inportb(GPIOP3DATA_ADDR)|0x10); // digitalWrite(_latch, HIGH); Latch data loaded during *prior* interrupt
  ...
  if((++plane) >= nPlanes) {    // Advance plane counter.  Maxed out?
    ...
    if(row & 0x1)   outportb(GPIOP4DATA_ADDR, inportb(GPIOP4DATA_ADDR)|0x80);    // digitalWrite(_a, HIGH);
    else            outportb(GPIOP4DATA_ADDR, inportb(GPIOP4DATA_ADDR)&(~0x80)); // digitalWrite(_a, LOW);
    if(row & 0x2)   outportb(GPIOP2DATA_ADDR, inportb(GPIOP2DATA_ADDR)|0x80);    // digitalWrite(_b, HIGH);
    else            outportb(GPIOP2DATA_ADDR, inportb(GPIOP2DATA_ADDR)&(~0x80)); // digitalWrite(_b, LOW);
    if(row & 0x4)   outportb(GPIOP4DATA_ADDR, inportb(GPIOP4DATA_ADDR)|0x20);    // digitalWrite(_c, HIGH);
    else            outportb(GPIOP4DATA_ADDR, inportb(GPIOP4DATA_ADDR)&(~0x20)); // digitalWrite(_c, LOW);
    if(nRows > 8) {
      if(row & 0x8) outportb(GPIOP3DATA_ADDR, inportb(GPIOP3DATA_ADDR)|0x08); // digitalWrite(_d, HIGH);
      else          outportb(GPIOP3DATA_ADDR, inportb(GPIOP3DATA_ADDR)&(~0x08)); // digitalWrite(_d, LOW);
    }
  }

  ptr = (uint8_t *)buffptr;

  outportb(GPIOP4DATA_ADDR, inportb(GPIOP4DATA_ADDR)&(~0x08)); // digitalWrite(_oe, LOW); Re-enable output
  outportb(GPIOP3DATA_ADDR, inportb(GPIOP3DATA_ADDR)&(~0x10)); // digitalWrite(_latch, LOW); Latch down

  if(plane > 0) {

	uint8_t val = inportb(GPIOP4DATA_ADDR);
	for (i=0; i<32; i++)
	{
		setPort2DATA((ptr[i] & 0xFC) >> 2);
		outportb(GPIOP4DATA_ADDR, val|0x02);//digitalWrite(_sclk, HIGH);
	    outportb(GPIOP4DATA_ADDR, val&(~0x02));//digitalWrite(_sclk, LOW);
	}
	...
	uint8_t val = inportb(GPIOP4DATA_ADDR);
    for(i=0; i<WIDTH; i++) {
      setPort2DATA((ptr[i] << 4) | ((ptr[i+WIDTH] << 2) & 0x0C) | (ptr[i+WIDTH*2] & 0x03));
      outportb(GPIOP4DATA_ADDR, val|0x02);//digitalWrite(_sclk, HIGH);
	  outportb(GPIOP4DATA_ADDR, val&(~0x02));//digitalWrite(_sclk, LOW);
    } 
  }
