...
char _command_QILOCIP[] = "AT+QILOCIP";
	...
	case 2:
		...
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp, aux))
	    {
			...
			if(resp)
			{
				...
				// AT+QILPORT
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QILPORT=\"TCP\",",false);
	...
	case 3:
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			if(resp)
			{
				...
				// Great. Go for the next step
				// AT+QISERVER
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QISERVER=0,",false);
...
bool GSM3ShieldV1MultiServerProvider::recognizeUnsolicitedEvent(byte oldTail)
{
	...
	prepareAuxLocate("0, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
	...
	//REMOTE SOCKET ACCEPTED.
	prepareAuxLocate("1, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
		theGSM3ShieldV1ModemCore.gss.cb.flush();
	...
	//REMOTE SOCKET ACCEPTED.
	prepareAuxLocate("2, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
		theGSM3ShieldV1ModemCore.gss.cb.flush();
	...
	//REMOTE SOCKET ACCEPTED.
	prepareAuxLocate("3, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
		theGSM3ShieldV1ModemCore.gss.cb.flush();
	...
	//REMOTE SOCKET ACCEPTED.
	prepareAuxLocate("4, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
		theGSM3ShieldV1ModemCore.gss.cb.flush();
	...
	//REMOTE SOCKET ACCEPTED.
	prepareAuxLocate("5, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
		theGSM3ShieldV1ModemCore.gss.cb.flush();
	...
	//REMOTE SOCKET ACCEPTED.
	prepareAuxLocate("6, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
		theGSM3ShieldV1ModemCore.gss.cb.flush();
	...
	//REMOTE SOCKET ACCEPTED.
	prepareAuxLocate("7, REMOTE IP", auxLocate);
	if(theGSM3ShieldV1ModemCore.gss.cb.locate(auxLocate))
	{
		//To detect remote socket closed for example inside socket data.
		theGSM3ShieldV1ModemCore.gss.cb.flush();
