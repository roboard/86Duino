...
{
	theGSM3ShieldV1ModemCore.genericCommand_rq("ATD",false);
...
void GSM3ShieldV1VoiceProvider::retrieveCallingNumberContinue()
{
    ...
    case 1:	
		theGSM3ShieldV1ModemCore.genericCommand_rq("AT+CLCC");
...
void GSM3ShieldV1VoiceProvider::answerCallContinue()
{
    ...
    case 1:
		// ATA ;
		theGSM3ShieldV1ModemCore.genericCommand_rq("ATA");
		theGSM3ShieldV1ModemCore.setCommandCounter(2);
		break;
	case 2:
...
void GSM3ShieldV1VoiceProvider::hangCallContinue()
{
    ...
    case 1:
		//ATH
		theGSM3ShieldV1ModemCore.genericCommand_rq("ATH");
		theGSM3ShieldV1ModemCore.setCommandCounter(2);
		break;
	case 2:
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
...
bool GSM3ShieldV1VoiceProvider::recognizeUnsolicitedEvent(byte oldTail)
{
	...
	char auxLocate [15];
	//RING.
	prepareAuxLocate("RING", auxLocate);
	...
	//CALL ACEPTED.
	prepareAuxLocate("+COLP:", auxLocate);
	if(theGSM3ShieldV1ModemCore.theBuffer().locate(auxLocate))
	{
		//DEBUG
	...
	//NO CARRIER.
	prepareAuxLocate("NO CARRIER", auxLocate);
	if(theGSM3ShieldV1ModemCore.theBuffer().locate(auxLocate))
	{
		//DEBUG
	...
	//BUSY.
	prepareAuxLocate("BUSY", auxLocate);
	if(theGSM3ShieldV1ModemCore.theBuffer().locate(auxLocate))
	{
		//DEBUG	
	...
	//CALL RECEPTION.
	prepareAuxLocate("+CLIP:", auxLocate);
	if(theGSM3ShieldV1ModemCore.theBuffer().locate(auxLocate))
	{
		theGSM3ShieldV1ModemCore.theBuffer().flush();
		setvoiceCallStatus(RECEIVINGCALL);
