...
char _command_CGATT[] = "AT+CGATT=";
char _command_SEPARATOR[] = "\",\"";
...
void GSM3ShieldV1DataNetworkProvider::attachGPRSContinue()
{
	...
	else if(ct==2)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				//AT+QIFGCNT
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QIFGCNT=0");
	...
	else if(ct==3)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			if(resp)
			{
				...
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QICSGP=1,\"",false);
	...
	else if(ct==4)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			if(resp)
			{
				...
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QIMUX=0");
	...
	else if(ct==5)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			if(resp)
			{
				...
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QIMODE=1");
	...
	else if(ct==6)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				// AT+QINDI=1
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QINDI=1");
	...
	else if(ct==8)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				// AT+QIREGAPP
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QIREGAPP");
	...
	else if(ct==9)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				// AT+QIACT	
				theGSM3ShieldV1ModemCore.genericCommand_rq("AT+QIACT");
...
void GSM3ShieldV1DataNetworkProvider::detachGPRSContinue()
{
	...
	case 2:
		char auxLocate[12];
		prepareAuxLocate("+PDP DEACT", auxLocate);
