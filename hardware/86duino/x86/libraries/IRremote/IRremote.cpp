...
static int mc = 3, md = 1; // It is TIMER_PIN's MC and MD (equal to use arduino_to_mc_md[][])
...
static bool init_mc_irq(void) {
	/* this part is moved to the wiring.cpp
	Set_MCIRQ(GetMCIRQ());
	if(irq_Init() == false) 
	{
...
void rcvInit(double us) {
	...
	if(oneshot == false)
	{
	    ...
	    if(us < 50L) us = 50L;
		mcpwm_SetWidth(mc, md, us*100.0, us/2.0*100.0);
	}
}
...
void IRsend::mark(int time) {
	...
	TimerEnablePWM(); // Enable pin 3 PWM output
...
void IRsend::space(int time) {
	...
	TimerDisablePWM(); // Disable pin 3 PWM output
	delayMicroseconds(time);
}

void IRsend::outPin(int pin) {
	iroutpin = pin;
}
...
void IRsend::enableIROut(int khz) {
  ...
  // See my Secrets of Arduino PWM at http://arcfn.com/2009/07/secrets-of-arduino-pwm.html for details.
    ...
    // Init H/W PWM
    io_DisableINT();
    if(mc_md_inuse[iroutpin] == 0)
	{
		mcpwm_ReloadPWM(mcs, mds, MCPWM_RELOAD_CANCEL);
		mcpwm_SetOutMask(mcs, mds, MCPWM_HMASK_NONE + MCPWM_LMASK_NONE);
		mcpwm_SetOutPolarity(mcs, mds, MCPWM_HPOL_NORMAL + MCPWM_LPOL_NORMAL);
		mcpwm_SetDeadband(mcs, mds, 0L);
		mcpwm_ReloadOUT_Unsafe(mcs, mds, MCPWM_RELOAD_NOW);
		
		mcpwm_SetWaveform(mcs, mds, MCPWM_EDGE_A0I1);
    ...
    io_DisableINT();
	mcpwm_SetWidth(mcs, mds, us*100.0, us/3.0*100.0);
	
    mcpwm_ReloadPWM(mcs, mds, MCPWM_RELOAD_PEREND);
    if(mc_md_inuse[iroutpin] == 0)
	{
		mcpwm_Enable(mcs, mds);
		mc_md_inuse[iroutpin] = 1;
    }
	io_RestoreINT();
}
...
void IRrecv::enableIRIn() {
	...
	mcpwm_Enable(mc, md);
	io_RestoreINT();  // enable interrupts
