...
char    monthDays[12] = { 31,  28,  31,  30,  31,  30,  31,  31,  30,  31,  30,  31};
	...
	delay(10);
		if (CC3KPrinter != 0) { CC3KPrinter->print("Block: (addr "); CC3KPrinter->print((uint16_t)buf, HEX); CC3KPrinter->print(", length "); CC3KPrinter->print(len, HEX); }
	//	traceFn("Block: (addr %p, length 0x%04x)", buf, len);

		while (count < len)
		{
			if (CC3KPrinter != 0) { CC3KPrinter->println(); CC3KPrinter->print((uint16_t)offset,HEX); CC3KPrinter->print(": "); }
	//		traceFn("\r\n  %p: ", offset);
			lineCount = 0;
			pAscii = (char*)&ascii;
			memset(ascii,' ',16);
			while ((count < len) && (lineCount < 8))
			{
				...
				if (CC3KPrinter != 0) { CC3KPrinter->print((*offset)>>4,HEX); CC3KPrinter->print((*offset++)&0x0F,HEX); CC3KPrinter->print(" "); }
	//			traceFn("0x%02bx ", *offset++);
			...
			while (lineCount++ < 8)
			{
				if (CC3KPrinter != 0) { CC3KPrinter->print("     "); }
	//			traceFn("     ");
			}
			if (CC3KPrinter != 0) { CC3KPrinter->print("  | "); CC3KPrinter->print(ascii); CC3KPrinter->print("|"); }
	// 		traceFn("  | %s|", ascii);
	delay(10);
		}
...
int sntp::GetSystemClockAsNTPTime(SNTP_Timestamp_t* ntpSystemTime) 
{
	...
	#ifdef CLOCK_DEBUG
		if (CC3KPrinter != 0) { CC3KPrinter->print(" GetSystemClockAsNTPTime: "); CC3KPrinter->print(ntpSystemTime->seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(ntpSystemTime->fraction,HEX); }
	#endif
	return 0;
}
...
SNTP_Timestamp_t* sntp::NTPGetTime(SNTP_Timestamp_t* ntpTime, bool local) 
{
	...
	if (ntpTime)
	{
		...
		*ntpTime = m_NTPReferenceTime;
		...
		#ifdef CLOCK_DEBUG
			if (CC3KPrinter != 0) { CC3KPrinter->print(" NTPGetTime m_NTPReferenceTime: "); CC3KPrinter->print(ntpTime->seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(ntpTime->fraction,HEX); }
		#endif
		
		AddNTPtime(&ntpSystemTime, ntpTime);

		#ifdef CLOCK_DEBUG
			if (CC3KPrinter != 0) { CC3KPrinter->print(" NTPGetTime NTP Current UTC: "); CC3KPrinter->print(ntpTime->seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(ntpTime->fraction,HEX); }
		#endif
		...
		if (local)                                         //if true, add in local offset from UTC
		{
			...
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print(" NTPGetTime NTP Current Local: "); CC3KPrinter->print(ntpTime->seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(ntpTime->fraction,HEX); }
			#endif
		}
	}
	return ntpTime;
...
NetTime_t *sntp::ExtractNTPTime(/*in*/ SNTP_Timestamp_t *ntpTime, /*out*/ NetTime_t *extractedTime)
{
	...
	dayno = time / SECS_DAY;                  //days since 1/1/1900 (or since 2/7/2036)
		...
		if (CC3KPrinter != 0) { CC3KPrinter->print("dayclock = "); CC3KPrinter->print(dayclock); CC3KPrinter->print("dayno = "); CC3KPrinter->println(dayno); }
	#endif
	...
	{                                       // back into dayno and dayclock.  They've got plenty of room for it now
		...
		#ifdef CLOCK_DEEP_DEBUG
			if (CC3KPrinter != 0) { CC3KPrinter->print("    date overflow: dayclock ="); CC3KPrinter->print(dayclock); CC3KPrinter->print("dayno = "); CC3KPrinter->println(dayno); }
		#endif
	...
	#ifdef CLOCK_DEEP_DEBUG
		if (CC3KPrinter != 0) { CC3KPrinter->print(year); CC3KPrinter->print("  = ("); CC3KPrinter->print(dayno); CC3KPrinter->print(" // "); CC3KPrinter->print(FIX_PT_DAYS_YEAR); CC3KPrinter->print(") + "); CC3KPrinter->println(BASE_YEAR); }
	#endif
...
char sntp::GetNTPServerList(const char** ntp_pool_list, uint32_t* addrBuffer, int maxServerCount)
{
	...
	if ((ntp_pool_list) && (addrBuffer))
	{
		...
		{
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print("Checking NTP server/pool address "); CC3KPrinter->println(*ntpPoolName); }
			#endif

			gethostbyname(*ntpPoolName, strlen(*ntpPoolName), &ntpServer);

			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print("     returns ntpServer: "); CC3KPrinter->println(ntpServer,HEX); }
			#endif
...
bool  sntp::SNTP_GetTime(int	sntpSocket, uint32_t *ntpServerAddr) 
{
	...
	#ifdef CLOCK_DEBUG
		if (CC3KPrinter != 0) { CC3KPrinter->println("SNTP_GetTime"); }
	#endif
	...
	#ifdef CLOCK_DEBUG
		if (CC3KPrinter != 0) { CC3KPrinter->print("SNTP sendto server "); CC3KPrinter->println(socketAddr.sin_addr.s_addr,HEX); }
	...
	#ifdef CLOCK_DEBUG
		if (CC3KPrinter != 0) { CC3KPrinter->print("sendto transmitted "); CC3KPrinter->print(byteCount,HEX); CC3KPrinter->println(" bytes"); }
	#endif
		
	if (sizeof(SNTP_Message_t) == byteCount)
	{
		#ifdef CLOCK_DEBUG
			if (CC3KPrinter != 0) { CC3KPrinter->println("Waiting for response"); }
		#endif
		...
		#ifdef CLOCK_DEBUG
			if (CC3KPrinter != 0) { CC3KPrinter->print("SNTP received "); CC3KPrinter->print(byteCount); CC3KPrinter->println(" bytes"); }
		#endif
		...
		if (sizeof(SNTP_Message_t) == byteCount)
		{
			...
			#ifdef CLOCK_DEBUG
				dumpBlock1((char*)&sntp_message, sizeof(SNTP_Message_t));
				if (CC3KPrinter != 0) { CC3KPrinter->println("Received at "); }
			...
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print(" (R - O): "); CC3KPrinter->print(sntp_message.tsReceive.seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(sntp_message.tsReceive.fraction,HEX); }
			#endif
			NTPdiv2(&sntp_message.tsReceive);
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print(" (R - O)/2: "); CC3KPrinter->print(sntp_message.tsReceive.seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(sntp_message.tsReceive.fraction,HEX); }
			#endif

			//Server sent reply at tsTransmit, we received reply at tsDestination
			DiffNTPtime(&tsDestination, &sntp_message.tsTransmit);
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print(" (T - D): "); CC3KPrinter->print(sntp_message.tsTransmit.seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(sntp_message.tsTransmit.fraction,HEX); }
			#endif
			NTPdiv2(&sntp_message.tsTransmit);
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print(" (T - D)/2: "); CC3KPrinter->print(sntp_message.tsTransmit.seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(sntp_message.tsTransmit.fraction,HEX); }
			#endif
			...
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print(" Offset: "); CC3KPrinter->print(sntp_message.tsTransmit.seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(sntp_message.tsTransmit.fraction,HEX); }
			#endif
			...
			m_timeIsSet = true;
				...
				if (CC3KPrinter != 0) { CC3KPrinter->print(" time is "); CC3KPrinter->print(tsDestination.seconds,HEX); CC3KPrinter->print("."); CC3KPrinter->println(tsDestination.fraction,HEX); }
			#endif
...
bool sntp::UpdateNTPTime()
{
	...
	#ifdef CLOCK_DEBUG
		if (CC3KPrinter != 0) { CC3KPrinter->println("sntp.update"); }
	#endif
	...
	if (sntpSocket >= 0)               //got one?
	{
		...
		{	//set up the sockAddr
			...
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print("local: "); CC3KPrinter->print(socketAddr.sin_addr.s_addr, HEX); CC3KPrinter->print(": "); CC3KPrinter->println(socketAddr.sin_port, HEX); }
			#endif
		}
		...
		{
			#ifdef CLOCK_DEBUG
				if (CC3KPrinter != 0) { CC3KPrinter->print("Init SNTP bind socket "); CC3KPrinter->print(sntpSocket); CC3KPrinter->print("to port "); CC3KPrinter->print(socketAddr.sin_port); CC3KPrinter->println(" succeeded"); }
			#endif
        ...
        m_globalPool = ntp_global_pool_list;
          ...
          if (CC3KPrinter != 0) 
          { 
            CC3KPrinter->print("m_localPool: "); CC3KPrinter->print(*m_localPool); CC3KPrinter->print("checkLocal: "); CC3KPrinter->println(checkLocal);
            CC3KPrinter->print("m_globalPool: "); CC3KPrinter->print(*m_globalPool); CC3KPrinter->print("checkGlobal: "); CC3KPrinter->println(checkGlobal);
          }
        #endif
        #ifdef CLOCK_DEBUG
					if (CC3KPrinter != 0) { CC3KPrinter->print("try user's ntp server list: "); CC3KPrinter->println((uint32_t)*ntp_pool_list); }
				#endif
				...
				while ((!m_timeIsSet) && (pServerList))
				{
					...
					if (!m_timeIsSet)
					{
						...
						if (checkLocal)
						{
							...
							checkLocal = false;
							#ifdef CLOCK_DEBUG
								if (CC3KPrinter != 0) { CC3KPrinter->print("try local ntp_pool_list: "); CC3KPrinter->println((uint32_t)*ntp_pool_list); }
							#endif
						}
						else
						{
							if (checkGlobal)
							{
								...
								checkGlobal = false;
								#ifdef CLOCK_DEBUG
								if (CC3KPrinter != 0) { CC3KPrinter->print("try global ntp_pool_list: "); CC3KPrinter->println((uint32_t)*ntp_pool_list); }
								#endif
							}
							else
							{
...
//
		...
		{
			if (CC3KPrinter != 0) { CC3KPrinter->print("Init SNTP bind socket "); CC3KPrinter->print(sntpSocket); CC3KPrinter->print("to port "); CC3KPrinter->print(SNTP_PORT); CC3KPrinter->println("failed"); }
			closesocket(sntpSocket);
			sntpSocket = -1;
		}
	} //if (sntpSocket >= 0)
	...
	{
		if (CC3KPrinter != 0) { CC3KPrinter->println("Failed to get a socket"); }
	}
	#ifdef CLOCK_DEBUG
		if (CC3KPrinter != 0)
		{
			CC3KPrinter->println("exit sntp.update");
			CC3KPrinter->print("m_std_UTC_offset.seconds: "); CC3KPrinter->println(m_std_UTC_offset.seconds, HEX);
			CC3KPrinter->print("m_dst_UTC_offset.seconds: "); CC3KPrinter->println(m_dst_UTC_offset.seconds, HEX);
		}
	#endif
	return m_timeIsSet;
}
