...
#define CHECK_SUCCESS(func,Notify,errorCode)  {if ((func) != CC3000_SUCCESS) { CHECK_PRINTER CC3KPrinter->println(Notify); return errorCode;}}
...
bool Adafruit_CC3000::scanSSIDs(uint32_t time)
{
  ...
  {
    CHECK_PRINTER {
      CC3KPrinter->println("Started AP/SSID scan\n\r");
    }
  }
...
bool Adafruit_CC3000::begin(uint8_t patchReq, bool useSmartConfigData, const char *_deviceName)
{
  ...
  if (g_IRQnum == 0xFF) {
    CHECK_PRINTER {
      CC3KPrinter->println("IRQ pin is not an INT pin!");
    }
    return false;
  }
  ...
  if (useSmartConfigData)
  {
    ...
    while(!cc3000Bitset.test(CC3000BitSet::IsConnected))
    {
      ...
      {
        CHECK_PRINTER {
          CC3KPrinter->println("Timed out using SmartConfig data");
        }
        return false;
      }
      timeout += 10;
...
bool Adafruit_CC3000::startSmartConfig(const char *_deviceName, const char *smartConfigKey)
{
  ...
  CHECK_PRINTER {
    CC3KPrinter->println("Got smart config data");
  }
  if (enableAES) {
  ...
  while(!cc3000Bitset.test(CC3000BitSet::IsConnected))
  {
    ...
    {
      CHECK_PRINTER {
        CC3KPrinter->println("Timed out waiting to connect");
      }
      return false;
    }
    timeout += 10;
...
bool Adafruit_CC3000::connectSecure(const char *ssid, const char *key, int32_t secMode)
{
  ...
  if ( (secMode < 0) || (secMode > 3)) {
    CHECK_PRINTER {
      CC3KPrinter->println("Security mode must be between 0 and 3");
    }
    return false;
  }
  ...
  if (strlen(ssid) > MAXSSID) {
    CHECK_PRINTER {
      CC3KPrinter->print("SSID length must be < ");
  ...
  if (strlen(key) > MAXLENGTHKEY) {
    CHECK_PRINTER {
      CC3KPrinter->print("Key length must be < ");
...
bool Adafruit_CC3000::connectToAP(const char *ssid, const char *key, uint8_t secmode, uint8_t attempts) {
  ...
  do {
    ...
    CHECK_PRINTER {
      CC3KPrinter->print("Waiting to connect...");
    }
...
uint16_t Adafruit_CC3000::ping(uint32_t ip, uint8_t attempts, uint16_t timeout, uint8_t size) {
  ...
  //if (CC3KPrinter != 0) {
  //  CC3KPrinter->print("Pinging "); printIPdots(revIP); CC3KPrinter->print(" ");
  //  CC3KPrinter->print(attempts); CC3KPrinter->println(" times");
  //}
  
  netapp_ping_send(&revIP, attempts, size, timeout);
  delay(timeout*attempts*2);
  //if (CC3KPrinter != 0) CC3KPrinter->println("Req report");
  //netapp_ping_report();
  //if (CC3KPrinter != 0) { CC3KPrinter->print("Reports: "); CC3KPrinter->println(pingReportnum); }

  if (pingReportnum) {
    /*
    if (CC3KPrinter != 0) {
      CC3KPrinter->print("Sent: "); CC3KPrinter->println(pingReport.packets_sent);
      CC3KPrinter->print("Recv: "); CC3KPrinter->println(pingReport.packets_received);
      CC3KPrinter->print("MinT: "); CC3KPrinter->println(pingReport.min_round_time);
      CC3KPrinter->print("MaxT: "); CC3KPrinter->println(pingReport.max_round_time);
      CC3KPrinter->print("AvgT: "); CC3KPrinter->println(pingReport.avg_round_time);
    }
    //*/
...
Adafruit_CC3000_Client Adafruit_CC3000::connectTCP(uint32_t destIP, uint16_t destPort)
{
  ...
  // Create the socket(s)
  //if (CC3KPrinter != 0) CC3KPrinter->print("Creating socket ... ");
  tcp_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  if (-1 == tcp_socket)
  {
    CHECK_PRINTER {
      CC3KPrinter->println("Failed to open socket");
    }
    return Adafruit_CC3000_Client();
  }
  //CC3KPrinter->print("DONE (socket "); CC3KPrinter->print(tcp_socket); CC3KPrinter->println(")"));
  ...
  //if (CC3KPrinter != 0) CC3KPrinter->print("Connecting socket ... ");
  if (-1 == ::connect(tcp_socket, &socketAddress, sizeof(socketAddress)))
  {
    CHECK_PRINTER {
      CC3KPrinter->println("Connection error");
    }
    closesocket(tcp_socket);
    return Adafruit_CC3000_Client();
  }
  //if (CC3KPrinter != 0) CC3KPrinter->println("DONE");
...
Adafruit_CC3000_Client Adafruit_CC3000::connectUDP(uint32_t destIP, uint16_t destPort)
{
  ...
  //if (CC3KPrinter != 0) { CC3KPrinter->print("DONE (socket "); CC3KPrinter->print(udp_socket); CC3KPrinter->println(")"); }
...
int Adafruit_CC3000_Client::available(void) {
  ...
  //if (CC3KPrinter != 0) } CC3KPrinter->print("Select: ")); CC3KPrinter->println(s); }
