...
#define SET_AUDIO_2205161 0x4110 //22.05kHz 1 channel
...
static BOOL hda_reset(void)
{
	...
	udelay(1000);
	printf("4.1\n");
	d0 = hda_rd_reg32(HDAGCTL);
	printf("4.2\n");
	d0 |= CRST;
	printf("4.3\n");
	hda_wr_reg32(HDAGCTL, d0);
	printf("4.4\n");	
	tmout = 1000;
	printf("5\n");
	while(((hda_rd_reg32(HDAGCTL) & CRST) == 0) && --tmout)	
...
static void hda_prepare(void)
{
	...
	//vic
	hda_wr_reg16(OSDFMT, hda_set_rate());	/*48 kHz, 16 bits per sample, 1 channels*/
...
static void common_pcm_upload_func(void)
{
	...
	while(d0 < g_current_req){
		...
		if(wd.pcm_format == _16BITSTEREO){
		...
		else if(wd.pcm_format == _16BITMONO){
			d1	= ((mixing_buff[(d0*2) + 0] * g_master_volume)/256);
			if(d1 >  32767) d1 =  32767;
			if(d1 < -32768) d1 = -32768;
			d3	= d1 & 0xFFFF;
			...
			*/
			d3 &= 0x0000FFFF;
			_farnspokel(g_wss_dma_addr + (g_write_cursor * 2), (short)d3);
		}
