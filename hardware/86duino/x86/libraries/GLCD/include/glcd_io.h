...
#define lcd_avrWriteByte(data) 					\
	do \
	{ \
		digitalWrite(glcdData0Pin, data & 0x01); \
		digitalWrite(glcdData1Pin, data & 0x02); \
		digitalWrite(glcdData2Pin, data & 0x04); \
		digitalWrite(glcdData3Pin, data & 0x08); \
		digitalWrite(glcdData4Pin, data & 0x10); \
		digitalWrite(glcdData5Pin, data & 0x20); \
		digitalWrite(glcdData6Pin, data & 0x40); \
		digitalWrite(glcdData7Pin, data & 0x80); \
	} \
	while(0)
	/*
...
#define lcd_avrReadByte() 					\
		digitalRead(glcdData0Pin) + (digitalRead(glcdData1Pin) << 1) + (digitalRead(glcdData2Pin) << 2) +  \
	    (digitalRead(glcdData3Pin) << 3) + (digitalRead(glcdData4Pin) << 4) + (digitalRead(glcdData5Pin) << 5) + \
	    (digitalRead(glcdData6Pin) << 6) + (digitalRead(glcdData7Pin) << 7)
...
#define lcdDataDir(dirbits)					\
	do \
	{ \
		pinMode(glcdData0Pin, (dirbits & 0x01) ? OUTPUT : INPUT); \
		pinMode(glcdData1Pin, (dirbits & 0x02) ? OUTPUT : INPUT); \
		pinMode(glcdData2Pin, (dirbits & 0x04) ? OUTPUT : INPUT); \
		pinMode(glcdData3Pin, (dirbits & 0x08) ? OUTPUT : INPUT); \
		pinMode(glcdData4Pin, (dirbits & 0x10) ? OUTPUT : INPUT); \
		pinMode(glcdData5Pin, (dirbits & 0x20) ? OUTPUT : INPUT); \
		pinMode(glcdData6Pin, (dirbits & 0x40) ? OUTPUT : INPUT); \
		pinMode(glcdData7Pin, (dirbits & 0x80) ? OUTPUT : INPUT); \
	} \
	while(0)
	/*
 ...
 */
#define lcdRdBusystatus()		(digitalRead(glcdData7Pin))
#define lcdRdResetstatus()		(digitalRead(glcdData4Pin))
...
#ifdef glcdRES
#define lcdReset()		digitalWrite(glcdRES, 0)
#define lcdUnReset()	digitalWrite(glcdRES, 1)
#else
#define lcdReset()		
#define lcdUnReset()		
#endif
...
#define lcdDelayNanoseconds(__ns) \
do \
{  \
