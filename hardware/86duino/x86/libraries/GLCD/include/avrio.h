 ...
 */
#define avrio_PinMode(pin, dir) 		pinMode(pin, dir)//avrio_WriteBit(AVRIO_DDRREG, pin, dir)
...
#define avrio_ReadPin(pin) 			digitalRead(pin)//avrio_ReadBit(AVRIO_PINREG, pin)
...
#define avrio_WritePin(pin, data)		digitalWrite(pin, data)//avrio_WriteBit(AVRIO_PORTREG, pin, data)
 ...
 */
#define avrio_digitalRead(pin)			digitalRead(pin)//avrio_ReadPin(pin)
#define avrio_digitalReadPin(pin)		digitalRead(pin)//avrio_ReadPin(pin)
...
#define avrio_digitalWrite(pin, data)		digitalWrite(pin, data)//avrio_WritePin(pin, data)
#define avrio_digitalWritePin(pin, data)	digitalWrite(pin, data)//avrio_WritePin(pin, data)
...
#define avrio_pinMode(pin, dir) pinMode(pin, dir)//avrio_PinMode(pin, dir)
...
#define AVRDATA_8BIT(p0, p1, p2, p3, p4, p5, p6, p7)    (false)
...
#define AVRDATA_4BITLO(p0, p1, p2, p3)    (false)

/*
 * figures out if 4 avrpins map to bits D4-D7 on an AVR port
 */
 
#define AVRDATA_4BITHI(p0, p1, p2, p3)    (false)
	...
	avrpin_t p4, avrpin_t p5, avrpin_t p6, avrpin_t p7, uint8_t data)
{
	if(regtyp == AVRIO_DDRREG)
	{
		pinMode(p0, (data & 0x01) ? OUTPUT : INPUT);
		pinMode(p1, (data & 0x02) ? OUTPUT : INPUT);
		pinMode(p2, (data & 0x04) ? OUTPUT : INPUT);
		pinMode(p3, (data & 0x08) ? OUTPUT : INPUT);
		pinMode(p4, (data & 0x10) ? OUTPUT : INPUT);
		pinMode(p5, (data & 0x20) ? OUTPUT : INPUT);
		pinMode(p6, (data & 0x40) ? OUTPUT : INPUT);
		pinMode(p7, (data & 0x80) ? OUTPUT : INPUT);
	}
	else
	{
		digitalWrite(p0, data & 0x01);
		digitalWrite(p1, data & 0x02);
		digitalWrite(p2, data & 0x04);
		digitalWrite(p3, data & 0x08);
		digitalWrite(p4, data & 0x10);
		digitalWrite(p5, data & 0x20);
		digitalWrite(p6, data & 0x40);
		digitalWrite(p7, data & 0x80);
	}
	/*
	...
	avrpin_t p4, avrpin_t p5, avrpin_t p6, avrpin_t p7)
{
	...
	data = digitalRead(p0) + (digitalRead(p1) << 1) + (digitalRead(p2) << 2) +
	       (digitalRead(p3) << 3) + (digitalRead(p4) << 4) + (digitalRead(p5) << 5) +
	       (digitalRead(p6) << 6) + (digitalRead(p7) << 7);
/*	       
	if(AVRDATA_8BIT(p0, p1, p2, p3, p4, p5, p6, p7))
	{
